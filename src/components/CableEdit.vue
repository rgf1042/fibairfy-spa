<template>
    <div>
        ...
        >
            <p class="my-4">
                Segur que vol esborrar el cable: {{ deleted.name }}
            </p>
        </b-modal>
        <b-modal
            id="modal-template-change"
            ref="changeModalRef"
            @ok="onChange"
            title="Template cable"
        >
            <p class="my-4">
                Segur que vol canviar la template? Es perdran tots els tubs i
                fibres.
            </p>
        </b-modal>
        <b-container>
...
export default {
    ...
    mounted() {
        let cable = this.$store.getters['projects/findCableById'](
    ...
    methods: {
        onSubmit(evt) {
            ...
            this.$store.dispatch('projects/updateCable', this.form).then(
                response => {
                    this.$router.go(-1);
                },
                error => {
        ...
        deleteCable() {
            this.$store.dispatch('projects/deleteCable', this.deleted.id).then(
                response => {
        ...
        addTube(evt) {
            ...
            this.$store.dispatch('projects/addNewTube', tube).then(
                response => {},
                error => {
                    this.alert.message = error.msg;
                    this.alert.show = true;
        ...
        onChange() {
            ...
            this.$store.dispatch('projects/setCableTemplate', data).then(
                response => {},
                error => {
                    this.alert.message = error.msg;
                    this.alert.show = true;
        ...
        onClear(evt) {
            evt.preventDefault();
            this.$store.dispatch('projects/clearTubesCable', this.form.id).then(
                response => {},
                error => {
                    this.alert.message = error.msg;
                    this.alert.show = true;
