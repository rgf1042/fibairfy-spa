<template>
    <div>
        ...
        <b-form-select
            ...
            :options="options"
            class="mb-3"
            :disabled="select !== null || !this.current.writable"
        />
        <b-button
            variant="danger"
...
export default {
    ...
    computed: {
        select: {
            get: function() {
                let fusion;
                if (this.type === 'fiber') {
                    fusion = this.$store.getters['projects/getFusionByFiber'](
                        this.id
                    );
                } else if (this.type === 'box') {
                    fusion = this.$store.getters['projects/getFusionByBox']({
                        id: this.id,
                        in: this.in,
                        out: this.out,
                    });
        ...
        options() {
            ...
            let output = this.$store.getters['projects/getPossibleFusions'](
                data
            );
            ...
            for (let key in boxes) {
                if (boxes.hasOwnProperty(key)) {
                    ...
                    for (let x in boxes[key].outputs) {
                        // For evry output
                        let item = {};
                        item.value = {
                            type: 'box',
    ...
    methods: {
        ...
        deleteFusion() {
            ...
            this.$store.dispatch('projects/deleteFusion', fusion).then(
                response => {},
                error => {
                    console.log(error);
                }
