...
function Pfusion(cnvs, vue) {
    ...
    this.cables =
        this.vue.$store.getters['projects/cablesIndexes'](this.site) || [];
    ...
    for (let x in this.cables) {
        ...
        this.tubesCables[cable] = this.vue.$store.getters[
            'projects/tubesIndexes'
        ](cable);
        for (let y in this.tubesCables[cable]) {
            let tube = this.tubesCables[cable][y];
            this.tubes[tube] = this.vue.$store.getters['projects/findTubeById'](
                tube
            );
            ++this.tubesCount;
        }
    ...
    for (let x in this.tubesCables) {
        if (this.tubesCables.hasOwnProperty(x)) {
            for (let y in this.tubesCables[x]) {
                ...
                this.fibersTubes[tube] = this.vue.$store.getters[
                    'projects/fibersIndexes'
                ](tube);
                for (let z in this.fibersTubes[tube]) {
                    let fiber = this.fibersTubes[tube][z];
                    this.fibers[fiber] = this.vue.$store.getters[
                        'projects/findFiberById'
                    ](fiber);
                }
            }
        }
    ...
    this.defaultMarginTube =
        (this.defaultWidthCable1Tube - this.defaultWidthTube) / 2;
    this.defaultMarginCables = 70;
    this.defaultMarginBox = 70;
    this.defaultWidthStrokeFiber = 3;
    this.defaultOffsetCables =
        this.defaultHeightBox +
        this.defaultMarginBox * 2 +
        this.defaultHeightFiber;
    this.defaultFusionOffset = 15;
    this.defaultFusionChunk = 6;
    this.fontsize = 14;

    let cablesWidth =
        this.defaultOffsetCables * 2 +
        (this.tubesCount *
            (this.defaultWidthTube + this.defaultMarginTube * 2) +
    ...
    this.defaultCanvasWidth =
        boxesWidth > cablesWidth ? boxesWidth : cablesWidth;
...
Pfusion.prototype.drawBoxes = function() {
    ...
    let left =
        this.defaultCanvasWidth -
        (this.defaultMarginBox + this.defaultWidthBox);
    let width = this.defaultWidthBox;
    let height = this.defaultHeightBox;

    for (let x in this.boxes) {
        let box = this.vue.$store.getters['projects/findBoxById'](
            this.boxes[x]
        );
...
Pfusion.prototype.drawFibersBoxes = function(box, rectBox) {
    ...
    for (let i = 1; i <= box.inputFO; ++i) {
        let lineFiber = new fabric.Line(
            [left, top, left, top + this.defaultHeightFiber],
            {
                stroke: 'grey',
                strokeWidth: this.defaultWidthStrokeFiber,
            }
        );
        this.fibersBoxesGraphics[box.id] =
            this.fibersBoxesGraphics[box.id] || {};
        this.fibersBoxesGraphics[box.id].inputs =
            this.fibersBoxesGraphics[box.id].inputs || {};
    ...
    for (let i = 1; i <= box.outputFO; ++i) {
        ...
        this.fibersBoxesGraphics[box.id] =
            this.fibersBoxesGraphics[box.id] || {};
        this.fibersBoxesGraphics[box.id].outputs =
            this.fibersBoxesGraphics[box.id].outputs || {};
...
Pfusion.prototype.drawCables = function() {
    ...
    for (let x in this.cables) {
        ...
        let name =
            'cable ' +
            this.vue.$store.getters['projects/findCableById'](cable).id;
        let rectCable;
        if (x % 2) {
            // odd
            // We draw it vertical
            let length = this.tubesCables[cable]
                ? this.tubesCables[cable].length
                : 1;
...
Pfusion.prototype.drawTubes = function(cable, rectCable) {
    if (rectCable.width / rectCable.height < 1) {
        // Vertical
    ...
    forEach(this.fusions, (fusion, i, fusions) => {
        ...
        if (direction1 === direction2) {
            ...
            if (direction1 === 'horizontal') {
                ...
                line1y2 =
                    y1 -
                    (this.defaultFusionChunk * number1 +
                        this.defaultFusionOffset);
            ...
            let lineFusion3 = new fabric.Line(
            ...
            if (
                (direction1 === 'box' && direction2 === 'horizontal') ||
                (direction2 === 'box' && direction1 === 'horizontal')
            ) {
                // TODO
                ...
                let line2y2;
                let random =
                    number1 * this.defaultFusionChunk * (Math.random() * 4);
                if (direction1 === 'box') {
                    avg = (y2 - y1) / 2;
