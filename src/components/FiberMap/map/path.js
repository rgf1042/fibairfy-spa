...
Path.prototype.save = function() {
    ...
    this.map_parent.vue.$store.dispatch('projects/addNewPath', path).then(
        response => {
    ...
    if (this.id) {
        ...
        switch (this.map_parent.layerActive) {
            ...
            case 'infra':
                ...
                if (this.cables.length > 0) this.showCablePopup(e.latlng);
                break;
            case 'fiber':
...
Path.prototype.onPathMouseOver = function(e) {
    if (this.map_parent.layerActive === 'civil' && this.id) {
        this.map_parent.info.update(
            this.map_parent.vue.$t('components.editPaths.path') +
                ': ' +
                this.name +
                '(' +
                this.id +
...
Path.prototype.drawCable = function(id) {
     ...
     */
    this.cables.push(id);
    if (this.polyline) this.changeTypePath(); // Canviem de color si fa falta
};
...
Path.prototype.showCablePopup = function(latlng) {
    ...
    for (let x in this.cables) {
        ...
        link.innerHTML =
            this.cables[x] +
            ' - ' +
            this.map_parent.getFiber(this.cables[x]).name;
