...
Fiber.prototype.getSites = function() {
    ...
    for (var x = 1; x < this.paths.length; x++) {
        ...
        if (
            prePath.first_site.id === actPath.first_site.id ||
            prePath.first_site.id === actPath.end_site.id
        ) {
            ...
            prePath.end_site.id === actPath.first_site.id ||
            prePath.end_site.id === actPath.end_site.id
        ) {
            if (this.sites.length === 0) {
                // Només pot passar la primer vegada
...
Fiber.prototype.getAllDots = function() {
    ...
    for (let x = 0; x < this.paths.length; x++) {
        ...
        if (actPath.first_site.id === this.sites[x]) {
            dots.push([
                actPath.first_site.latitude,
                actPath.first_site.longitude,
            ]);
            for (let y = 0; y < actPath.intermedial.length; y++) {
                ...
                if (dots.length == 0) {
                    ...
                    if (
                        beforeDot[0] != actDot[0] ||
                        beforeDot[1] != actDot[1]
                    ) {
                        dots.push(actDot);
                    }
                }
...
Fiber.prototype.addSite = function(site) {
    ...
    var pathBetween = this.map_parent.getPathBeetwenSites(
        lastSiteInFiber,
        site.id
    );
...
Fiber.prototype.save = function() {
    ...
    this.map_parent.vue.$store.dispatch('projects/addNewCable', cable).then(
        response => {
