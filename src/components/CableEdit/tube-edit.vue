<template>
    <div>
        ...
        >
            <p class="my-4">
                Segur que vol esborrar el tub: {{ deleted.name }}
            </p>
        </b-modal>
        <b-row class="pt-2">
            <b-col cols="3">
                <h3>
                    {{ $tc('components.editTubes.editTube.title', 1) }}:
                    {{ form.id }}
                </h3>
            </b-col>
            <b-col cols="1">
    ...
    methods: {
        onSubmit() {
            if (this.form.id && this.changes) {
                this.$store.dispatch('projects/updateTube', this.form).then(
                    response => {},
                    error => {
                        console.log(error);
                    }
        ...
        deleteTube() {
            ...
            if (this.deleted.id) {
                this.$store
                    .dispatch('projects/deleteTube', this.deleted.id)
                    .then(
                        response => {
        ...
        addFiber(evt) {
            ...
            this.$store.dispatch('projects/addNewFiber', fiber).then(
                response => {},
                error => {
                    this.alert.message = error.msg;
                    this.alert.show = true;
