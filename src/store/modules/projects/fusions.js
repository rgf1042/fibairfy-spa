...
export default {
    ...
    getters: {
        getPossibleFusions: (state, getters) => data => {
            ...
            if (data.box) {
                ...
                if (data.box.id) {
                    ...
                    if (data.box.in) {
                        Vue.delete(
                            result.boxes[data.box.id].inputs,
                            data.box.in
                        );
                    } else if (data.box.out) {
                        Vue.delete(
                            result.boxes[data.box.id].outputs,
                            data.box.out
                        );
                    }
                ...
                if (state.fusions.data.hasOwnProperty(x)) {
                    ...
                    for (let y in fusion) {
                        if (
                            !couple ||
                            !(
                                fusion[y].type === couple.type &&
                                couple.id === fusion[y].id
                            )
                        ) {
                            // We test the pair
                            if (fusion[y].type === 'fiber') {
                                ...
                                if (fusion[y].in) {
                                    Vue.delete(
                                        result.boxes[fusion[y].id].inputs,
                                        fusion[y].in
                                    );
                                } else if (fusion[y].out) {
                                    Vue.delete(
                                        result.boxes[fusion[y].id].outputs,
                                        fusion[y].out
                                    );
                                }
                            }
        ...
        getFusionByFiber: state => fiber => {
            ...
            if (fusionPos)
                return state.fusions.data[fusionPos.id][fusionPos.pos];
            else return null;
        },
    ...
    mutations: {
        ...
        addNewFusion(state, fusion) {
            Vue.set(state.fusions.data, fusion.id, [
                fusion.fdata,
                fusion.sdata,
            ]);
            if (fusion.fdata.type === 'box') {
                ...
                if (fusion.fdata.in) {
                    ...
                    Vue.set(boxes[box]['inputs'], fusion.fdata.in, {
                        id: fusion.id,
                        pos: 1,
                    });
                } else if (fusion.fdata.out) {
                    Vue.set(boxes[box], 'outputs', boxes[box]['outputs'] || {});
                    Vue.set(boxes[box]['outputs'], fusion.fdata.out, {
                        id: fusion.id,
                        pos: 1,
                    });
                }
            ...
            if (fusion.sdata.type === 'box') {
                ...
                if (fusion.sdata.in) {
                    // If it's an input
                    Vue.set(boxes[box], 'inputs', boxes[box]['inputs'] || {});
                    Vue.set(boxes[box]['inputs'], fusion.sdata.in, {
                        id: fusion.id,
                        pos: 0,
                    });
                } else if (fusion.sdata.out) {
                    Vue.set(boxes[box], 'outputs', boxes[box]['outputs'] || {});
                    Vue.set(boxes[box]['outputs'], fusion.sdata.out, {
                        id: fusion.id,
                        pos: 0,
                    });
                }
                    ...
                    .then(
                        response => {
                            ...
                            for (let x in cables) {
                                ...
                                for (let y in tubes) {
                                    let tube = tubes[y];
                                    let fibers = context.getters.fibersIndexes(
                                        tube
                                    );
                                    for (let z in fibers) {
                                        let fiber = context.getters.findFiberById(
                                            fibers[z]
                                        );
                                        context.commit(
                                            'addFiberToFusions',
