                ...
                Vue.http
                    ...
                    .then(
                        response => {
                            ...
                            project.writable =
                                typeof response.body.users.find(
                                    item =>
                                        item.id ===
                                        context.rootGetters['user/currentId']
                                ) === 'object';
                            context.commit('setCurrentProject', project);
                            context.dispatch('map/setLocation', {
                                latitude: project.latitude,
                                longitude: project.longitude,
                            });
                            context.dispatch('map/setZoom', project.zoom);
                            // We load sites
                            context.dispatch('loadSites').then(
                                response => {
                                    context.dispatch('loadPaths').then(
                                        response => {
                                            context.dispatch('loadBoxes').then(
                                                response => {
                                                    context
                                                        .dispatch('loadCables')
                                                        .then(
                    ...
                    .then(
                        response => {
                            ...
                            for (let x in response.body) {
                                response.body[x].writable =
                                    typeof response.body[x].users.find(
                                        item =>
                                            item.id ===
                                            context.rootGetters[
                                                'user/currentId'
                    ...
                    .then(
                        response => {
                            ...
                            if (context.getters.current.id === project) {
                                context.dispatch('unsetCurrent').then(
                                    response => {
                                        resolve(response);
                                    },
                                    error => {
